...
source("summary.R")
```
In the summary section, we wanted to explore the number of people of different races who were tested positive for Covid-19. First off, we figured out the number of features and obervations of the dataset by creating a list. Then we seperately calculated the number of state that has had the highest and lowest number of cases until 10/28/2020, as well as state that has had the highest number of deaths until 10/28/2020. The sate with the highest number of cases is California and the state with the lowest number of cases is Alaska. Next we figured out the total number of white Americans, African Americans, Asian Americans, Hispanic Americans, and multiracial Americans who testes positive for Covid-19.
## Table
```{r}
...
```{r}
avg_weekly_cases_age
```
The purpose of including this chart is to see how different age groups are affected by Covid-19, and to see which groups contribute most the number of cases present. In this plot we are able to that the most mean number of weekly cases is from the age 20-39 group, this group is also has the max number of cases in a pparticular week during the time of data collection

#### Chart 3:
```{r}

```
The purpose of this scatterplot chart is to see the changes of number of daily new cases between April and October. In this scatter plot table, we are able to tell the highest number of daily new cases by looking at the peak of the chart, which is in July. By looking at the entire graph, we are also able to tell the general growth trend of daily new cases over time with the month of April, July, and October seeing the most amount of people who tested postive for Covid-19.
